...
Seven of the eleven `LibKa0s` majors are adopted (`Core`, `Env`, `Media`, `DebugLog`, `Slash`,
`Options`, `Launcher`); `Pool`, `Item` and `Widgets` are vendored but not consumed here, and `Perf` is
