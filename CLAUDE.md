...
— into `libs/LibKa0s/`, and its test kit into `tests/_kit/`. Seven of the eleven majors are adopted
(`Core`, `Env`, `Media`, `DebugLog`, `Slash`, `Options`, `Launcher`); `Perf` is declined. **Never edit anything
...
no-combat-path criterion (a) fails: `modules/Canvas.lua:644-650` runs a shared 10Hz `OnUpdate`.
