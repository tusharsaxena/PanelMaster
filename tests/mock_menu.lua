-- COPIED FROM LibKa0s v1.58.0 (../LibKa0s tests/mock_menu.lua), byte for byte below this block.
-- The library keeps it repo-local rather than in the kit, so a host that tests its options menu
-- carries its own copy (the library's CHANGELOG, "What a consumer owes on re-vendoring v1.58.0").
-- Not a suite: tests/test_launcher.lua and tests/test_disabled.lua build it over the shared mocks.
--
