# 01 — Delta: PanelMaster vs LibKa0s v1.15.0
...
```sh
grep -in 'bundles' PanelMaster/CLAUDE.md
```
...
```sh
grep -hoE 'local +(MAJOR, )?[A-Z_]*MINOR *= *("[^"]+", *)?[0-9]+' PanelMaster/libs/LibKa0s/*.lua
```
...
```sh
diff -r --strip-trailing-cr <scratch>/LibKa0s PanelMaster/libs/LibKa0s   # 0 lines
diff -r                     <scratch>/LibKa0s PanelMaster/libs/LibKa0s   # 0 lines
diff -r --strip-trailing-cr <scratch>/testkit PanelMaster/tests/_kit     # 0 lines
diff -r                     <scratch>/testkit PanelMaster/tests/_kit     # 0 lines
```
...
```sh
grep -rnoE 'LibStub\("LibKa0s-[A-Za-z]+-1\.0", true\)' PanelMaster --include='*.lua' | grep -v '/libs/' | grep -v '/tests/'
```

- `PanelMaster/core/CoreSetup.lua:34` — Core
- `PanelMaster/core/DebugLogSetup.lua:102` — DebugLog
- `PanelMaster/core/EnvSetup.lua:40` — Env
- `PanelMaster/core/MediaSetup.lua:42` — Media
- `PanelMaster/settings/OptionsSetup.lua:26` — Options
- `PanelMaster/settings/Slash.lua:283` — Slash
...
```sh
grep -n 'Kit.VERSION' <scratch>/testkit/framework.lua PanelMaster/tests/_kit/framework.lua
```
...
| Gate | Command | Result |
|---|---|---|
| Lint | `luacheck .` | 0 warnings / 0 errors in 27 files |
| Tests | `lua tests/run.lua` | 731 passed, 0 failed, 0 skipped |
