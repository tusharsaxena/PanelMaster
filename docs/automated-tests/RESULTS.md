...
| 1000–1500 (on notice) | `settings/PanelEditor.lua` | 1464 | **Peeled after this run: PM-ATS-02 (ATS-10, #47), on the automated-tests sweep branch.** The editor's six tabs — the tab names and order, the editor's vertical rhythm and `buildPanelEditor` — moved whole into `settings/PanelEditorTabs.lua` (798) along the seam #47 verified, leaving `settings/PanelEditor.lua` at 746 with the page: the selection, the mutation actions, the control kit, the strip, the chrome band, the rebuilder and the bus. Each publishes its half on `NS.PanelEditor` and binds the other's on first use; 964 cases before and after, run in the same order. Both files leave the band at the next run. The ruling below is what the run recorded. **The trigger has fired, and it is tracked: issue [#47](https://github.com/tusharsaxena/PanelMaster/issues/47) (open).** Not a re-accept — the split is the open issue. **Back up to 1464** at `20260926-160448` (1447 at `20260924-104311`, +17 net from `3a7ac19` pm-band-fix-01, the Panels band row layout), which leaves **36 lines of headroom** under `layout-§1`'s 1500 cap; it is the largest authored non-test file in the repository. Its densest function is still low (the addon's two CCN-15 functions remain `core/Compat.lua@27-45` `Compat.AddOnFolders` and `modules/Registry.lua@611-639` `R.ApplyArtSize`, both at the threshold, neither above it). The file is moving toward the cap again after one small fix; #47 is the next thing that happens to it, before the next feature lands here. |
