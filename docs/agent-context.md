# Agent Context — Ka0s Panel Master

**This is the Ka0s Addon Context Pack (v2.7.0, 2026-07-26), dropped into this addon at scaffold time.** It is the full agent brief: stack, layout, hard rules, invariants, starter patterns and the Definition of Done. Self-contained — no external lookups required for an LLM or a new contributor to work on this addon and stay compliant.

Start from the root [CLAUDE.md](../CLAUDE.md) "Standards compliance (read first)" section, then this file, then [ARCHITECTURE.md](ARCHITECTURE.md) for what this particular addon does with all of it.
