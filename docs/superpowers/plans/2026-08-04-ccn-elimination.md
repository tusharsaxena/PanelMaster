...
8. `local function buildSectionQuads(sections, n, u0, u1, fv0, fv1, width, height, point, x, y, flipH, flipV, rotation, tile)` for lines 1007-1046, taking its inputs POSITIONALLY. CCN ~9. An earlier draft of this step passed ONE ctx table assembled at the call site; it was reverted because it added a hash table to every composed BuildArtSpec call — measured at +616 bytes/call, +10.8% on the composite path (5680 -> 6296 bytes/call over 100k iterations with the collector stopped) — in exchange for nothing but a shorter signature. The extraction must not allocate: a fifteen-argument call list is the correct price for a helper on a render path. `span` is derived inside from `u0`/`u1`, and the caller passes the already-mapped `fv0`/`fv1` rather than `toFileV` plus `v0`/`v1`, so no closure crosses the boundary either.
