...
| `locales/enUS.lua` | `NS.L` | The canonical locale table and its key-is-the-string fallback. Carries no keys in 1.0.0 — see **Localization**. |
...
| `settings/PanelEditor.lua` | `NS.PanelEditor`, `E.TABS` | The Panels page's body: the create box, the panel selector, the editor's **five-tab strip** and the section builder behind each tab, the page's mutation actions and its two bus triggers. The page is bespoke — a panel is a registry record, not a schema row — so the strip is drawn straight onto the chrome band with `H.TabStrip` (`options-ui-§13`) and dispatched on `ctx.activeTab`, rather than partitioned by `H.RenderTabbedSchema`. **Nothing that acts on the panel as a whole is a tab**: creating, picking, renaming, copying, Enabled, Unlock, Reset and Delete all live in the chrome band above the strip (`options-ui-§14`) and stay put on every tab. Peeled out of `settings/Panel.lua` (`layout-§1`) and drawn with that file's helpers, which it reads from `NS.Panel.__ui`. |
