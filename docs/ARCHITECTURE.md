...
|---|---|---|---|---|
| `documentation-§1` (item 5) | `README.md` carries no `## What's new` section. `## Version History` (item 12) is present. | **`0.1.0` is the first release, so there is nothing new to report yet.** The section's job is to tell a returning player what changed since the build they already had; on an initial release every reader is a new one, and the whole README is the answer. The section it would duplicate — the feature list — already sits above it. Restored at the **first update**, which is the first moment it can say anything true. Owner's decision, 2026-08-07. | 2026-08-07 | The first version bump after `0.1.0` — `/wow-addon:bump-version` rolls this section and `## Version History` together, so that command is the trigger and it retires this row |
