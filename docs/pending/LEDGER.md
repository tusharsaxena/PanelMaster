...
| GIT-01 | 6d67c2c6 | `git log origin/master..HEAD` | 🟡 deferred | 2026-07-31 | Release held. Three commits stay local until the in-client check is done. Superseded 2026-07-31: `origin/master` is now level with `HEAD`, so the item no longer surfaces. The branch was `origin/main` when this row was written; renamed to `origin/master` 2026-07-31, locator updated to stay reproducible. Evidence hash unchanged — the locator is not the evidence text. |
