...
| `core/State.lua` | `NS.State` | Session-only runtime state: `debug`, `unlocked`, `unlockedPanels`. Never persisted. Everything here keyed by panel id is dropped by `Registry:ReloadProfile`, because ids are allocated per profile. |
...
Nine of the fifteen `LibKa0s` majors are adopted (`Core`, `Env`, `Media`, `DebugLog`, `Slash`,
`Options`, `Launcher`, `Lifecycle`, `Schema`); `Pool`, `Item` and `Widgets` are vendored but not
consumed here, nor are `Compat` (no member fits, closed issue
[#53](https://github.com/tusharsaxena/PanelMaster/issues/53)) and `Bus` (its `Catalog` is optional
here and deferred, [#52](https://github.com/tusharsaxena/PanelMaster/issues/52)), and
