...
| # | Decision | Rationale |
|---|---|---|
| A-001 | **Per-character** AceDB profile (`:New(name, defaults)` with no `defaultProfile`), where the sibling collection addons are account-wide. | A panel layout belongs to the UI a given character runs. `savedvariables` does not mandate a scope; AceDB's profile machinery still lets a user copy one layout to an alt. **⚠ Reversed later the same day** at the user's request: the addon now passes `true` and every character starts on the shared **"Default"** profile, bringing it in line with the sibling addons. The reasoning inverted — most people run one UI, so the shared default is the one that costs nobody work, and a per-character layout is two clicks away on the new Profiles page. See `ARCHITECTURE.md` ▸ Data model. |
