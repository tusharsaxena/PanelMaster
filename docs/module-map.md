...
| `core/Database.lua` | `NS:InitDB`, `NS:RunMigrations`, `NS:SweepPreviewPanels` | AceDB open on the shared "Default" profile, the migration seam, the preview-orphan sweep, the profile-change callbacks, the `[Init]` summary. `NS:SweepPreviewPanels` is the panel registry's `architecture-§5` **load pass**, called only from `NS:InitDB` and the profile callbacks. |
...
| `modules/Registry.lua` | `NS.Registry` | **Owns `db.profile.panels`**, and is its `architecture-§5` **registry writer**: the only runtime writer of membership, `nextID`, `id` and `frameName`. Create, delete, rename, reset, copy-from, field edits, sanitizing, name and frame-name uniqueness (the latter on create only — see *Frame names and the pool*), off-screen recovery, profile reload. Sole sender of both panel messages. |
