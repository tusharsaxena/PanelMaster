...
| Master controls | Minimap button | Show this addon's button on the minimap. Unlike the two rows above it, this one is **remembered** — a button you hide stays hidden across a reload, across a profile switch, and across **both** resets on this page — the **Defaults** button in the header and **Reset all settings** below. Where the button sits is a per-installation display preference, like the angle you dragged it to, and no reset touches either (`launcher-§3`). Untick it and the button goes at once; LibDBIcon's own right-click menu writes the same setting, so the two always agree. Hovering the button shows its status tooltip, drawn by `LibKa0s-Launcher-1.0` (`launcher-§1`, standard v2.66.0) and shown while the addon is disabled too: `Ka0s Panel Master  v<version>`, **Enabled** and **Locked** (Yes/No, green/red), `Left-click: Unlock frame` / `Lock frame` (while disabled: `disabled — /pm enable`) and `Right-click: Open settings`. There is no **Test mode** line, because this addon has no test mode. |
