...
-- ── The Panels page's editor tabs ───────────────────────────────────────────────
-- What is ON each of the editor's six tabs, for one panel: the tab names and their strip order,
-- the editor's vertical rhythm, and buildPanelEditor, which draws the active tab's controls.
--
-- Peeled out of settings/PanelEditor.lua (layout-§1, PanelMaster#47), which keeps the page: the
-- selection, the page's mutation actions, the control kit these tabs are drawn with, the strip, the
-- chrome band, the rebuilder and the bus. The page reaches this file through one call,
-- buildPanelEditor, plus the strip's two tab tables; this file reaches the page through the control
-- kit and the page's two published seams, E.__pageActions and E.__panelsByName. Each side publishes
-- its half on E and binds the other's on first use, the way both bind settings/Panel.lua's helpers,
-- so the two files may load in either order.

-- ── Helpers shared with the page ────────────────────────────────────────────────
-- Bound on first use rather than captured at load, for the reason settings/PanelEditor.lua gives:
-- settings/Panel.lua loads after both editor files, and the page's own kit may load after this one.
local attachTooltip, makePairButton, trackDropdown, BUTTON_PAIR_REL
local editorRow, editorSpacer, editorHeading, addRefresher
local makeMediaDropdown, makeColorPair, makeEdgeChecks
local pageAction, panelsByName

local function bindHelpers()
  pageAction, panelsByName = E.__pageActions, E.__panelsByName
  local kit = E.__controls
  if kit then
    editorRow,         editorSpacer,  editorHeading,  addRefresher   = kit.editorRow,
      kit.editorSpacer, kit.editorHeading, kit.addRefresher
    makeMediaDropdown, makeColorPair, makeEdgeChecks                 = kit.makeMediaDropdown,
      kit.makeColorPair, kit.makeEdgeChecks
  end
  local ui = NS.Panel and NS.Panel.__ui
  if not ui then return end
  attachTooltip, makePairButton = ui.attachTooltip, ui.makePairButton
  trackDropdown, BUTTON_PAIR_REL = ui.trackDropdown, ui.BUTTON_PAIR_REL
end
...
--
-- The middle size came back, and with it a heading. The editor's five SUBJECTS are tabs and are
-- announced by the strip -- that has not changed -- but a tab that mixes two kinds of control has
-- to say where one stops and the next starts (options-ui-§7), and three of the five do.
...
local function buildPanelEditor(ctx, parent, rec)
  ...
  sections[TAB_FADE] = function()
    ...
    -- Delete and Reset are not repeated here: they are page-wide acts and live in the chrome band
    -- above the strip, reachable from every tab at once rather than from the one that happened to
    -- be about the panel's identity.
    editorSpacer(group, EDITOR_ROW_GAP)
  end
  ...
  --
  -- Keyed off EDITOR_TABS[1], which is the same expression drawTabStrip heals `ctx.activeTab`
  -- with. It used to name TAB_GENERAL, and that constant is gone; naming a different one here
  -- would put the rule in two places and let the fallback and the strip disagree about which tab
  -- is first.
  local build = sections[ctx.activeTab] or sections[EDITOR_TABS[1]]
  build()

  parent:AddChild(group)
end

-- ── What the page drives this file with ─────────────────────────────────────────
-- The strip's order and membership tables (settings/PanelEditor.lua's drawTabStrip heals
-- `ctx.activeTab` with them) and the editor itself. EDITOR_TABS is also E.TABS, above.
E.__editorTabs = {
  EDITOR_TABS      = EDITOR_TABS,
  IS_EDITOR_TAB    = IS_EDITOR_TAB,
  buildPanelEditor = buildPanelEditor,
}
