...
| `core/LauncherSetup.lua` | `NS.Launcher` | The `LibKa0s-Launcher-1.0` seam (`launcher-§1`): **one** LibDataBroker object, registered twice — with LibDBIcon for the minimap button, and to any broker display the player runs. Both surfaces dispatch into one `OnClick`, so `launcher-§2`'s click rule is satisfied on both by construction rather than by two implementations agreeing. Since Launcher minor 4 (standard v2.67.0) left-click opens the settings panel and right-click opens the options menu, whose entries for this addon are **Enabled · Locked** (the standard's `ADDONS.md` row): `setEnabled` → `NS.Slash:CliEnable` (`/pm enable` / `disable`) and `toggleLock` → `NS.Slash:CliLock` (`/pm lock` / `unlock`), each beside its `isEnabled` / `isLocked` accessor; the descriptor never holds a copy of either state. No test mode (unlocking is the preview) and no primary window. Hovering shows the library-drawn status tooltip: the descriptor passes `version`, `isEnabled` and `isLocked`, and no `isTestMode` or `onTooltipShow`. `minimap` is passed as a **function**, not a table: `db.global.minimap` does not exist at file load and a table captured then is one AceDB later replaces. Degrades three ways, all silent and none fatal: no LibKa0s (a stub answering every member), no LibDataBroker (no object at all), no LibDBIcon (the broker plugin without the button). |
...
| `modules/Unlock.lua` | `NS.Unlock` | Unlock mode (outline, label, drag, snap), global and per-panel. It is also this addon's test mode (`options-ui-§15`), so Lock frame is its switch and there is no separate preview. `Unlock:PendingSnapshot()` is a read-only copy of the combat unlock queue, for the diagnostics report. |
| `modules/Diagnostics.lua` | `NS.Diagnostics` | The diagnostics report's sections (`debug-logging-§14`), in report order through `Dx.Sections()`: state, master, unlock queue, settings, screen, panels, frames, mouseover, artwork, events. The markers, the identity header, the per-section `pcall`, the cap and the ungated append are the library's. **Read-only**: it answers through the pure half of the code that acts and never writes, moves, shows, recovers or replays. Loads after `modules/Unlock.lua`; everything it reads is resolved through `NS` at call time. See [debug.md](debug.md). |
...
| `settings/PanelEditor.lua` | `NS.PanelEditor` | The Panels page's body: the create box, the panel selector, the editor's **six-tab strip**, the control kit every tab is drawn with (`E.__controls`: the row, spacer and heading makers, the refresher registry and the media, color-pair and edge makers), the page's mutation actions and its two bus triggers. What each tab holds is `settings/PanelEditorTabs.lua`'s. The page is bespoke — a panel is a registry record, not a schema row — so the strip is drawn straight onto the chrome band with `H.TabStrip` (`options-ui-§13`) and dispatched on `ctx.activeTab`, rather than partitioned by `H.RenderTabbedSchema`. **The acts that apply to the panel whole are the `General` first tab**: renaming, copying another panel's look, Enabled, Unlock, Reset and Delete sit there rather than under a subject tab, which `options-ui-§14` (standard v2.40.0) allows because it is the tab the page opens on. The chrome band above the strip is bounded at **one row** — the **Panel** picker and the **Create new panel** box — and those two stay put on every tab. Peeled out of `settings/Panel.lua` (`layout-§1`) and drawn with that file's helpers, which it reads from `NS.Panel.__ui`. |
| `settings/PanelEditorTabs.lua` | `E.TABS`, `E.__editorTabs` | The editor's six tabs: their names and strip order, the editor's vertical rhythm, and `buildPanelEditor`, which draws the active tab's controls for one panel. Peeled out of `settings/PanelEditor.lua` (`layout-§1`, [#47](https://github.com/tusharsaxena/PanelMaster/issues/47)). The page reaches it through `buildPanelEditor` and the strip's two tab tables; it reaches the page through `E.__controls`, `E.__pageActions` and `E.__panelsByName`, and `settings/Panel.lua`'s helpers through `NS.Panel.__ui`, all bound on first use. |
...
- `core/CoreSetup.lua` after `core/Namespace.lua` (which defines `NS.PREFIX`, passed to the printer
  ...
  `settings/PanelEditor.lua`, `settings/PanelEditorTabs.lua`, `settings/Panel.lua` — or the swap silently no-ops while appearing to
  work.
...
- `settings/OptionsSetup.lua` after `settings/Schema.lua` and `settings/Slash.lua`, and **before**
  ...
  its helpers lazily inside its own rebuild, so it pins nothing. `settings/PanelEditorTabs.lua` binds
  lazily too, at the top of `buildPanelEditor`, so the two editor files may load in either order.
...
| `tests/mock_menu.lua` | Not a suite. A headless stand-in for the client's context-menu API (`MenuUtil`), copied from LibKa0s v1.58.0's own `tests/mock_menu.lua`, for the launcher menu cases in `test_launcher.lua` and `test_disabled.lua`. |
...
| `tests/test_diagnostics.lua` | The diagnostics report's sections: order, the stood-down run, the unlock queue left intact, the per-panel lines, a raising section or panel, the cap, a secret value, and that running it writes nothing. The kit's `test_diagnostics_contract.lua` runs the shared contract against the real dispatcher. |
...
| `tests/test_launcher.lua` | The Launcher seam: the broker object, the two clicks (left opens settings, right opens the options menu), the menu's Enabled and Locked entries and their slash handlers, the status tooltip (enabled and disabled), the minimap row, the degraded install. |
